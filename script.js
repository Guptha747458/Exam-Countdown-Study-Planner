function savePlan() {
    const plan = {
      ...
      subTopics: document.getElementById("subTopics").value.split(','),
...
function showPreviousPlans() {
    ...
    plans.forEach((plan, index) => {
      ...
      item.innerHTML = `${plan.examName} - ${plan.examDate} <button onclick="deletePlan(${index})">Delete</button>`;
...
function generateFlowchart(plan) {
    ...
    plan.topics.forEach((topic, i) => {
      const topicId = `T${i}`;
      diagram += `${topicId}["${topic} (${plan.topicTime})"]\n`;
